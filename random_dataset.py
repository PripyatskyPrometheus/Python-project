    ...
    with open('random_number_dataset.csv', 'w+', encoding='utf-8', newline='') as file:
        ...
        for i in range(1, len(paths_txt)):
            ...
            while os.path.isfile(new_name):
                new_name = str(random.randint(0, 10001)).zfill(5) + '.txt'
            writer.writerow([f'{ (path_random_number_dataset) }\{ new_name }', f'..\\random_number_dataset\{new_name}', f'{class_txt[1]}'])
            shutil.copyfile(
                path_dataset + str(paths_txt[i]), path_random_number_dataset + '\\' + new_name)
